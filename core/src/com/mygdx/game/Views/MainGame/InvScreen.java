...
public class InvScreen implements Screen {
    ...
    public InvScreen(MyGdxGame parent, SpriteBatch batch) {
        ...
        phone = new Image(A2DAssetManager.manager.get(A2DAssetManager.phone, Texture.class));
    ...
    public void show() {
        ...
        for (int i = 0; i < parent.getGameScreen().getMainManager().getThings().size(); i++){
            ...
            if (parent.getGameScreen().getMainManager().getThings().get(i) == 6){
                phone.setBounds(1120 * w, (500 - (i * 120)) * k, 100 * w, 100 * k);
                table.addActor(phone);
            }
        }
