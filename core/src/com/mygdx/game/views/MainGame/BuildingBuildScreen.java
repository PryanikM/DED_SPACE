...
import com.mygdx.game.manager.BuildingManager;
...
//        table.add(dayCount);
   ...
   /* public void updateBusyPlayers(int ind){
        Gdx.app.log("way", "qwe");
        Label work = new Label("Busy", skin, "big");
        work.setFontScale(0.5f * k);
        work.setBounds(buttons.get(ind).getImageX() + 10 * k, buttons.get(ind).getImageY(), 93 * k, 150 * k);
        buttons.get(ind).setColor(Color.GRAY);
        buttons.get(ind).getListeners().clear();
        playerTable.addActor(work);
    }*/
    public void update(boolean check, int ind){
        if(check){
            Gdx.app.log("check", "tr");
        ...
        for (int i = 0; i < buttons.size(); i++){
            if (!main.getPlayers().get(i).getIsBusy()) {
                Gdx.app.log("Arrays", String.valueOf(manager.whatPressed(i)));
                if (manager.whatPressed(i)) {
                    Gdx.app.log("white", "no");
