...
import com.mygdx.game.makePlayers;
...
public class GameMainScreen implements Screen, InputProcessor {
    ...
    private makePlayers information;
...
//        tiledMap = new TmxMapLoader().load("Map/map1..tmx");
        ...
        if (!load) {
            information = new makePlayers(parent.getChoiceScreenPlayers().getArray(), k, (TiledMapTileLayer) tiledMap.getLayers().get("Tile Layer 1"), this);
            setPlayers();
//            savefirst();
...
//            hudScreen.updateCountDay();
        ...
        /*if (screenX >= 550  * k  && screenX <= 990  * k && screenY <= 300 * k && screenY >= 200 * k){
            if (!gameManager.getOuting()) {
                parent.changeScreen(MyGdxGame.OUTINGF);
            }
            else {
                parent.changeScreen(MyGdxGame.OUTINGT);
        ...
        /*if (screenX > 670 * k && screenX < 900 * k && screenY > 10 * k && screenY < 100 * k){
            Gdx.app.log("myTag", "aay");
        }*/
        return false;
    }
...
//        Gdx.app.log("CHECK", String.valueOf(gameManager.getBuildings().size()));
        for (int i = 0; i < gameManager.getBuildings().size(); i++){
            Sprite temp;
...
//        Gdx.app.log("CHECK", String.valueOf(gameManager.getTombstones().size()));
